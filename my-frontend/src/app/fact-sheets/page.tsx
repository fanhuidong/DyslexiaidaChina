          ...
          {factSheets && (factSheets as FactSheet[]).length > 0 ? (
            (factSheets as FactSheet[]).map((item) => {
              ...
              return (
                <div 
                  key={item.id} 
                  className="group flex flex-col md:flex-row md:items-center justify-between p-6 rounded-xl border border-gray-100 hover:border-gray-300 hover:shadow-md transition-all duration-200 bg-white"
                >
                  {/* 左侧：图标 + 信息 */}
                  <div className="flex items-start gap-4 mb-4 md:mb-0">
                    <div className="w-12 h-12 rounded-full bg-red-50 text-[#b91c1c] flex items-center justify-center shrink-0">
                      <FileText size={24} />
                    </div>
                    <div>
                      <h3 className="text-lg font-bold text-gray-900 group-hover:text-[#b91c1c] transition-colors">
                        {item.Title}
                      </h3>
                  ...
                  {fileUrl ? (
                    <a 
                      ...
                      className="flex items-center justify-center gap-2 px-6 py-2.5 rounded-lg border-2 border-transparent bg-gray-50 text-gray-700 font-bold hover:bg-[#b91c1c] hover:text-white transition-all group-hover:shadow-sm text-sm whitespace-nowrap"
                    >
                      <Download size={16} />
